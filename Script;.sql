...
Select max(total_cases) as TotalCases, max(total_deaths) as TotalDeaths, max(total_deaths)/max(total_cases) * 100 as DeathPercentage from CovidDeaths;

Select Location, Population, max(total_cases) as TotalCases, max(total_deaths) as TotalDeaths from CovidDeaths
WHERE Location IN ('World', 'High Income',
'Upper middle income','Europe', 
'Asia', 'North America', 
'South America', 'Oceania', 
...
'Asia', 'North America', 
'South America', 'Oceania', 
'Lower middle income', 'European Union', 'Africa', 'Low income')
Group By location, population
order by 1, 2;