...
-- Vaccination Per Location Percentage

with PopVac (location, Population, New_vaccinations, RollingVaccinationsPerLocation)
as 
(
Select deaths.location, deaths.population, vax.new_vaccinations,
SUM(vax.new_vaccinations) OVER (Partition by deaths.Location order by deaths.location, deaths.date)
as RollingVaccinationsPerLocation
From CovidDeaths deaths
Join CovidVaccinations vax
...
select *, (Max(RollingVaccinationsPerLocation)/population) * 100 as RollingVaccinationsPerLocationPercentage
from PopVac
group by location, population, new_vaccinations
